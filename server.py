...
def api_health_check(): return {'ok':True,'version':'47.0'}
...
def driver_register(p:DriverRegisterPayload):
    ...
    if not name or '@' not in email or not phone or len(password)<6: raise HTTPException(400,'full names, valid email, phone and password (6+ chars) required')
    salt=secrets.token_hex(16); ph=hashlib.pbkdf2_hmac('sha256',password.encode(),salt.encode(),200000).hex()
    with conn() as c:
        existing=c.execute('SELECT name FROM drivers WHERE lower(trim(email))=?',(email,)).fetchone()
    ...
    return {'ok':True,'driver':{'name':name,'driverId':'','email':email,'phone':phone,'selfRegistered':True}}
