...
- Driver login and registration now include a Show/Hide password control.
- First-time registration passwords require 6+ characters, including at least one letter, one number, and one special character.
- The backend enforces the same registration rule; returning drivers can continue using existing passwords.
